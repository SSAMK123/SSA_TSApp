...
### Actual vs Predicted Option
    ...
    if filter_by_project:
        ...
        if project_filter == "All Projects":
            st.subheader("Main Table Data")
            ###################################
            #adding filter if they select the ALL Projects to main table using group by
            avg_work = filtered_data.groupby(["Employee Name","Project Name"])["Time(in hours)"].mean()
            st.dataframe(avg_work)
            st.subheader("Project Resouce Table")
        ...
        if project_filter != "All Projects":
            ...
            st.subheader("Project Resouce Table")
            st.dataframe(filtered_project_resource_table)
